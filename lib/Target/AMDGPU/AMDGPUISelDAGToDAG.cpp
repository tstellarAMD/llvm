...
private:
  ...
  bool SelectMUBUFConstant(SDValue Constant,
                           SDValue &SOffset,
                           SDValue &ImmOffset,
                           bool CanUseSOffset = true) const;
  bool SelectMUBUFIntrinsicOffset(SDNode *Parent, SDValue Offset,
                                  SDValue &SOffset, SDValue &ImmOffset,
                                  bool CanUseSOffset = true) const;
  bool SelectMUBUFIntrinsicOffsetNoSOff(SDNode *Parent, SDValue Offset,
                                        SDValue &ImmOffset) const;
  bool SelectMUBUFIntrinsicVOffset(SDNode *Parent, SDValue Offset,
                                   SDValue &SOffset, SDValue &ImmOffset,
                                   SDValue &VOffset,
                                   bool CanUseSOffset = true) const;
  bool SelectMUBUFIntrinsicVOffsetNoSOff(SDNode *Parent, SDValue Offset,
                                         SDValue &ImmOffset,
  ...
  void SelectTBUFFER_STORE_FORMAT_PTR(SDNode *N) const;
...
void AMDGPUDAGToDAGISel::Select(SDNode *N) {
  ...
  case AMDGPUISD::TBUFFER_STORE_FORMAT_PTR:
    SelectTBUFFER_STORE_FORMAT_PTR(N);
    break;
  }
...
static bool isZero(const SDValue &Op) {
  auto C = dyn_cast<ConstantSDNode>(Op);
  return C && C->isNullValue();
}

void AMDGPUDAGToDAGISel::SelectTBUFFER_STORE_FORMAT_PTR(SDNode *N) const {

  SDLoc DL(N);
  SDValue Chain = N->getOperand(0);
  SDValue Rsrc = N->getOperand(2);
  SDValue VData = N->getOperand(1);
  EVT ValVT = VData.getValueType();
  SDValue NumChans = CurDAG->getConstant(
      !ValVT.isVector() ? 1 : alignTo(ValVT.getVectorNumElements(), 2),
      DL, MVT::i32);
  SDValue VIndex = N->getOperand(3);
  SDValue Offset = N->getOperand(4);
  SDValue SOffset = N->getOperand(5);
  SDValue Dfmt = N->getOperand(6);
  SDValue Nfmt = N->getOperand(7);
  SDValue Idxen = CurDAG->getConstant(!isZero(VIndex), DL, MVT::i32);
  SDValue Glc = N->getOperand(8);
  SDValue Slc = N->getOperand(9);
  SDValue Tfe = CurDAG->getConstant(0, DL, MVT::i32);

  SDValue InstOffset = CurDAG->getConstant(0, DL, MVT::i32);
  SDValue VOffset = Offset;

  SDValue Offen = CurDAG->getConstant(!isZero(VOffset), DL, MVT::i32);
  SDValue VAddr = SDValue(
      CurDAG->getMachineNode(TargetOpcode::IMPLICIT_DEF, DL, MVT::i32), 0);

  if (!isZero(Idxen) && !isZero(Offen)) {
    VAddr = CurDAG->getNode(ISD::BUILD_VECTOR, DL, MVT::v2i32, VAddr, VIndex);
  } else if (!isZero(Idxen)) {
    VAddr = VIndex;
  } else if (!isZero(Offen)) {
    VAddr = VOffset;
  }
  ...
  SDValue Ops[] = {
    ...
    Offen,
    Idxen,
    CurDAG->getZExtOrTrunc(Glc, DL, MVT::i32),
    CurDAG->getZExtOrTrunc(Slc, DL, MVT::i32),
    Tfe
  };

  CurDAG->MorphNodeTo(N, AMDGPUISD::TBUFFER_STORE_FORMAT, N->getVTList(), Ops);
}
...
bool AMDGPUDAGToDAGISel::SelectMUBUFIntrinsicOffset(SDNode *Parent,
                                                    SDValue Offset,
                                                    SDValue &SOffset,
                                                    SDValue &ImmOffset,
                                                    bool CanUseSOffset) const {
  ...
  return SelectMUBUFConstant(Offset, SOffset, ImmOffset, CanUseSOffset);
}

bool
AMDGPUDAGToDAGISel::SelectMUBUFIntrinsicOffsetNoSOff(SDNode *Parent,
                                                     SDValue Offset,
                                                     SDValue &ImmOffset) const {
  SDValue SOffset;

  return SelectMUBUFIntrinsicOffset(Parent, Offset, SOffset, ImmOffset, false);
}

bool AMDGPUDAGToDAGISel::SelectMUBUFIntrinsicVOffset(SDNode *Parent,
                                                     SDValue Offset,
                                                     SDValue &SOffset,
                                                     SDValue &ImmOffset,
                                                     SDValue &VOffset,
  ...
  if (isa<ConstantSDNode>(Offset)) {
    ...
    // bug.
    if (Subtarget->getGeneration() > AMDGPUSubtarget::SEA_ISLANDS ||
        SelectMUBUFConstant(Offset, Tmp1, Tmp2, CanUseSOffset))
      return false;
  }
  ...
  if (CurDAG->isBaseWithConstantOffset(Offset)) {
    ...
    if (cast<ConstantSDNode>(N1)->getSExtValue() >= 0 &&
        SelectMUBUFConstant(N1, SOffset, ImmOffset, CanUseSOffset)) {
      VOffset = N0;
      return true;
    }
  }
...
bool
AMDGPUDAGToDAGISel::SelectMUBUFIntrinsicVOffsetNoSOff(SDNode *Parent,
                                                     SDValue Offset,
                                                     SDValue &ImmOffset,
                                                     SDValue &VOffset) const {
  SDValue SOffset;
  return SelectMUBUFIntrinsicVOffset(Parent, Offset, SOffset, ImmOffset,
                                     VOffset, false);
}
