...
unsigned SIRegisterInfo::getAddressableNumSGPRs(const SISubtarget &ST) const {
  if (ST.getGeneration() >= AMDGPUSubtarget::VOLCANIC_ISLANDS)
    return 102;
  return 104;
}

unsigned SIRegisterInfo::getReservedNumSGPRs(const SISubtarget &ST) const {
  if (ST.getGeneration() >= AMDGPUSubtarget::VOLCANIC_ISLANDS)
...
unsigned SIRegisterInfo::getMaxNumSGPRs(const SISubtarget &ST,
  ...
  if (ST.getGeneration() >= AMDGPUSubtarget::VOLCANIC_ISLANDS) {
    switch (NumActiveWavesPerEU) {
      ...
      case 9:  return 80;
      case 8:  return 96;
      default: return getAddressableNumSGPRs(ST);
    }
  } else {
    switch (NumActiveWavesPerEU) {
      case 0:  return 48;
...
unsigned SIRegisterInfo::getMaxNumSGPRs(const MachineFunction &MF) const {
  ...
  std::pair<unsigned, unsigned> NumActiveWavesPerEU =
    MFI.getNumActiveWavesPerEU();
  unsigned MaxNumSGPRs = getMaxNumSGPRs(ST, NumActiveWavesPerEU.first);

  // Check if maximum number of SGPRs was explicitly requested using
  // "amdgpu-num-sgpr" attribute.
  if (F.hasFnAttribute("amdgpu-num-sgpr")) {
    unsigned Requested =
      AMDGPU::getIntegerAttribute(F, "amdgpu-num-sgpr", MaxNumSGPRs);

    // Make sure requested value does not violate subtarget's specifications.
    if (Requested && Requested <= getReservedNumSGPRs(ST))
      Requested = 0;

    // Make sure requested value is compatible with values implied by
    // default/requested minimum/maximum number of active waves per execution
    // unit.
    if (Requested && Requested > getMaxNumSGPRs(ST, NumActiveWavesPerEU.first))
      Requested = 0;
    if (NumActiveWavesPerEU.second &&
        Requested && Requested < getMinNumSGPRs(ST, NumActiveWavesPerEU.second))
      Requested = 0;
  ...
  return MaxNumSGPRs - getReservedNumSGPRs(ST);
}

unsigned SIRegisterInfo::getDebuggerReservedNumVGPRs(
...
unsigned SIRegisterInfo::getMinNumVGPRs(unsigned NumActiveWavesPerEU) const {
  switch (NumActiveWavesPerEU) {
    case 0:  return 0;
    case 10: return 0;
    case 9:  return 25;
...
unsigned SIRegisterInfo::getMaxNumVGPRs(unsigned NumActiveWavesPerEU) const {
  switch (NumActiveWavesPerEU) {
    case 0:  return 24;
    case 10: return 24;
    case 9:  return 28;
...
unsigned SIRegisterInfo::getMaxNumVGPRs(const MachineFunction &MF) const {
  ...
  std::pair<unsigned, unsigned> NumActiveWavesPerEU =
    MFI.getNumActiveWavesPerEU();
  unsigned MaxNumVGPRs = getMaxNumVGPRs(NumActiveWavesPerEU.first);

  // Check if maximum number of VGPRs was explicitly requested using
  // "amdgpu-num-vgpr" attribute.
  if (F.hasFnAttribute("amdgpu-num-vgpr")) {
    unsigned Requested =
      AMDGPU::getIntegerAttribute(F, "amdgpu-num-vgpr", MaxNumVGPRs);

    // Make sure requested value does not violate subtarget's specifications.
    if (Requested && Requested <= getDebuggerReservedNumVGPRs(ST))
      Requested = 0;

    // Make sure requested value is compatible with values implied by
    // default/requested minimum/maximum number of active waves per execution
    // unit.
    if (Requested && Requested > getMaxNumVGPRs(NumActiveWavesPerEU.first))
      Requested = 0;
    if (NumActiveWavesPerEU.second &&
        Requested && Requested < getMinNumVGPRs(NumActiveWavesPerEU.second))
      Requested = 0;
  ...
  return MaxNumVGPRs - getDebuggerReservedNumVGPRs(ST);
}
